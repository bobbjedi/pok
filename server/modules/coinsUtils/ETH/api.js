...
let updateGasPriceGlobal,
    gasPrice;
module.exports = seed => {
    const {ADDRESS, privateKey} = createKeyPair(seed);
     ...
     */
        ...
        async getBalance(address = ADDRESS){
            return (await eth.getBalance(address)) / ethSat; 
        },

        get FEE() {
            return gasPrice * 22000 / ethSat * 2;
        },
        getNonce() {
            return new Promise(resolve => {
        ...
        async send(params, contract) {
            try {
                console.log('ETH balance:', await this.getBalance());
                const txParams = {
                    nonce: this.currentNonce++,
                    // nonce: await this.getNonce() + 1,
                    gasPrice,
    ...
    // Init
    // TODO: грязно! переписать на класс
    updateGasPriceGlobal = updateGasPriceGlobal || ETH.updateGasPrice;
    ETH.getNonce();
    updateGasPriceGlobal();
    setInterval(() => {
        updateGasPriceGlobal();
    }, 10 * 1000);
